try {
    const TAB_MESSAGE_TIMEOUT_MS = 5e3;
    const CAPTURE_API_TIMEOUT_MS = 15e3;
    const RESTORE_MESSAGE_TIMEOUT_MS = 3e3;
    ...
    function sendCaptureMessage(tabId, message, timeoutMessage, timeoutMs = TAB_MESSAGE_TIMEOUT_MS) {
        return withTimeout(browser.tabs.sendMessage(tabId, message), timeoutMs, timeoutMessage);
    }
    async function captureWithRect(rect, tabId = null) {
        try {
            ...
            let dataUrl;
            if (tabId) {
                dataUrl = await withTimeout(browser.tabs.captureTab(tabId, options), CAPTURE_API_TIMEOUT_MS, "Timed out while capturing the tab screenshot");
            } else {
                dataUrl = await withTimeout(browser.tabs.captureVisibleTab(null, options), CAPTURE_API_TIMEOUT_MS, "Timed out while capturing the visible tab screenshot");
            }
            return dataUrl;
        } catch (error) {
    ...
    async function captureViewport(tabId = null) {
        ...
        return withTimeout(browser.tabs.captureVisibleTab(null, options), CAPTURE_API_TIMEOUT_MS, "Timed out while capturing the visible viewport");
    }
    async function captureByScrollAndStitch(dimensions, options = {}) {
        ...
        try {
            const prepared = await sendCaptureMessage(tabId, {
            ...
            }, "Timed out while preparing the page for dynamic capture");
            ...
            if (hideOverlays) {
                const overlayResult = await sendCaptureMessage(tabId, {
                    action: "hideCaptureOverlays"
                }, "Timed out while hiding page overlays");
            ...
            while (!reachedBottom && rowIndex < maxScrolls) {
                ...
                for (const x of grid.xPositions) {
                    throwIfCancelled(options);
                    const scrollInfo = await sendCaptureMessage(tabId, {
                        action: "scrollToCapturePosition",
                        x: x,
                        y: y,
                        waitMs: waitMs
                    }, "Timed out while scrolling the page during dynamic capture", Math.max(TAB_MESSAGE_TIMEOUT_MS, waitMs + 3e3));
            ...
            try {
                ...
                }, "Timed out while restoring the page after dynamic capture", RESTORE_MESSAGE_TIMEOUT_MS);
            } catch (restoreError) {
